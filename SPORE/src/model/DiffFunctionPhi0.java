	...
	public double valueAt(double[] arg0) {
		...
		for (int z = 0; z < Paras.K; z++) {
			...
			for (int w = 0; w < sp.W; w++) {
				//System.out.println(this.phi0.length+" "+sp.phitopic.length+" "+ sp.phitopic[z].length+" "+z+" "+w );
				//double temp = this.phi0[z] + sp.phitopic[z][w];作者这里应该写错了
				double temp = this.phi0[w] + sp.phitopic[z][w];
