...
public class SPORE {
	...
	public void updateTheta0() {
		System.out.println("cur update theta 0.....");
	...
	public void updateThetaPre() {
		...
		try {
			...
			// TODO Auto-generated catch block
			System.out.println("[error]update theta user.....");
			e.printStackTrace();
		}
	}
	...
	public void recommend_baseAll(String testFile) {
		...
		try {
			...
			for (int i = 0; i < maxthreads; i++) {
				RecommenderBaseAll b = new RecommenderBaseAll(joblists[i], i);
				executor.submit(b);
			}
			executor.shutdown();
			try {
		 ...
		 System.out.println("the total number of records where there is lessthan 20 items is: "+this.countTestRandom);
	}
	 ...
	 */
	public void lyh_make_perinfer() throws NumberFormatException, IOException {
		System.out.println("make fzvs :");
//		this.Dvw = new ArrayList[this.V];
//		for (int v = 0; v < this.V; v++) {
//			this.Dvw[v] = new ArrayList<Integer>();
//		}
...
//		reader.close();
		...
		for (int z = 0; z < K; z++) {
			for (int w = 0; w < this.W; w++) {
				this.betas[z][w] = this.inferBeta(z, w);
			}
			for (int v = 0; v < this.V; v++) {
				this.gammas[z][v] = this.inferGamma(z, v);
			}
		}
		System.out.println("FZVs is :");
		for (int z = 0; z < K; z++) {
			for (int v = 0; v < this.V; v++) {
				double fzv = 0;
				double ratingw = 0;
				ArrayList<Integer> words = this.Dvw[v];
				if (words.size() != 0) {
					int w = words.get(0);
					ratingw = this.betas[z][w];
					double wordSize = words.size();
					for (int wi = 1; wi < wordSize; wi++) {
						w = words.get(wi);
						ratingw += (this.betas[z][w]);
					}
